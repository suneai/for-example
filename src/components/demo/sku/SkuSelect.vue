<template lang="html">
  <div class="skuModule">
    ...
    <div class='select_type' v-for="(item,index) in skuType" :key="index">
      <SelectItem :selectType="item" :typeName="index" @type-select-change="typeSelectChange"></SelectItem>
    </div>
  </div>
</template>
...
export default {
  ...
  data(){
    return {
      ...
      skuList: [
        {'color':'银色', 'size': '小号','config':'高配'},
        {'color':'银色', 'size': '大号','config':'中配'},
        {'color':'玫瑰金', 'size': '小号','config':'低配'},
        {'color':'银色', 'size': '小号','config':'中配'},
        {'color':'玫瑰金', 'size': '大号','config':'中配'}
      ],
      //存储类别
      skuType:{},
      typeIsSelected:{},
